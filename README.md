...
> **Window 외 운영체제에서 `docker.exe` 설치 시** `https://docs.docker.com/desktop/` 에 접속하여 설치 진행바랍니다. 
...
1. 폴더 창 주소줄(위쪽)에 `powershell`이라고 치고 엔터를 누르세요.
   > **팁:** `Cmd`나 다른 터미널도 되지만, 파일이 있는 폴더에서 여는 것이 중요합니다! (위 순서대로 진행시 Download 폴더에 저장됩니다)
2. 파란 화면(터미널)이 나오면 아래 명령어를 입력하세요.
